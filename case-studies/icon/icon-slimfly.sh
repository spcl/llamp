...
num_procs=256
output_file="/users/sshen/workspace/lgs-mpi/case-studies/icon/run.out"
...
TRACE_LIB=/scratch/sshen/LogGOPSim/src/liballprof2/liballprof_f77.so
...
# -------------------------------
thisdir=$(pwd)
basedir="/scratch/snx3000/sshen/icon/icon-src"
echo $basedir
...
experiments_dir="/scratch/snx3000/sshen/icon/results"
# export ${basedir}
icon_data_rootFolder="/scratch/snx3000/sshen/icon/data"
...
# =========================== SLURM ===========================
# Default distribution is packed
distribution="block"
if [ -n "$rankmap" ]; then
    # If rankmap file is not specified
    distribution="arbitrary"
    export SLURM_HOSTFILE="$rankmap_file"
fi
export START="srun --export=ALL -m $distribution --cpu-bind=v"
# =========================== MPICH ===========================
# rank_file="/users/sshen/workspace/run_scripts/icon_rank"
# export START="${MPIRUN} -np $mpi_procs_pernode -rf ${rank_file}"
# export START="mpirun --envall -env INJECTED_LATENCY 0 -env MPICH_ASYNC_PROGRESS 1 -env UCX_RNDV_THRESH 256000 -env UCX_RC_VERBS_SEG_SIZE 256000 -f /scratch/sshen/lgs-mpi/case-studies/icon/hosts -map-by core -np $num_procs $rankmap"
# =========================== OpenMPI ===========================
...
start_date=${start_date:="2000-01-01T00:00:00Z"}
            end_date=${end_date:="2000-01-01T06:00:00Z"}
