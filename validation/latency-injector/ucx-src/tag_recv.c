        ...
        const ucp_request_param_t *param, const char *debug_name)
{
	...
	ucs_assert(rdesc->recv_time.tv_sec >= 0 && rdesc->recv_time.tv_nsec >= 0);

	// If the message was found in the unexpected queue
	// Add the timestamp to the req struct
	req->recv.tag.info.recv_time.tv_sec = rdesc->recv_time.tv_sec;
	req->recv.tag.info.recv_time.tv_nsec = rdesc->recv_time.tv_nsec;
	/* printf("[DEBUG] Found in unexp, creating req, recv ts: %ld:%ld\n", req->recv.tag.info.recv_time.tv_sec, req->recv.tag.info.recv_time.tv_nsec); */
    }
    ...
    ucs_assert(req->recv.tag.info.recv_time.tv_sec > 0);
