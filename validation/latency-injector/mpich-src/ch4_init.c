...
/** <Variables used by the delay request handler thread> **/
int delay_queue_read = 0;
int delay_queue_write = 0;
atomic_int terminate_req_thread = 0;
pthread_t delay_req_handler;
pthread_mutex_t req_thread_lock;

int delay = -1;
atomic_int queue_size = 0;
struct req_wrapper delay_queue[MAX_QUEUE_SIZE] = { 0 };
...
/**
 * Dequeues the delay requests and executes the corresponding
 * complete functions.
 */
void *handle_delay_reqs(void *arg)
{
  /* printf("[DEBUG] Starting the delay request handler thread\n"); */
  // Pins the thread to the last CPU core
  ...
  if (rc != 0)
  {
    printf("[ERROR] Failed to pin delay request handler thread to the last core!\n");
    pthread_exit(NULL);
  }
  ...
  while (!terminate_req_thread)
  {
    // Polls the queue
    if (delay_queue[delay_queue_read].req != NULL)
    {
      // If an request is present
      struct req_wrapper *delay_req = &delay_queue[delay_queue_read];
      // Only adds delay to RECV requests
      if (delay > 0 && delay_req->req->kind == MPIR_REQUEST_KIND__RECV)
      {
	long end_sec = delay_req->end.tv_sec;
	long end_ns = delay_req->end.tv_nsec;
	// Compares timestamps
	while (true)
	{
	  // Keeps looping until the `end` timestamp is reached
	  clock_gettime(CLOCK_MONOTONIC, &curr_time);
	  if (((curr_time.tv_sec > end_sec) ||
	       (curr_time.tv_sec == end_sec && curr_time.tv_nsec >= end_ns)) ||
	      terminate_req_thread)
	    break;
	}
      }
      // Checks the completion type
      if (!delay_req->is_fast_complete)
      {
	// If the request requires regular completion
	MPID_Request_complete_impl(delay_req->req);
      }
      else
      {
	// If the request requires fast completion
	MPIDI_Request_complete_fast_impl(delay_req->req);
      }
      delay_req->req = NULL;
      delay_queue_read = (delay_queue_read + 1) % MAX_QUEUE_SIZE;
      
      queue_size--;
      MPIR_Assert(queue_size >= 0);
      /* printf("[DEBUG] Removed request from queue: %d\n", delay_queue_read); */
    } // ** if (delay_queue[delay_queue_read].req != NULL) **
  }
  /* printf("[DEBUG] Delay request thread exiting...\n"); */
  pthread_exit(NULL);
}
...
int MPID_Init(int requested, int *provided)
{
    ...
    if (delay < 0)
    {
      char *delay_str = getenv("INJECTED_LATENCY");
      if (delay_str)
      {
        delay = atoi(delay_str);
	/* printf("[DEBUG] INJECTED_LATENCY: %s\n", delay_str); */
	if (delay > MAX_DELAY)
	{
	  printf("[ERROR] Injected latency cannot be larger than 1 sec!\n");
	  exit(1);
	}
      }
    }
    // Ensures that all requests are NULL
    for (int i = 0; i < MAX_QUEUE_SIZE; ++i)
    {
      (&delay_queue[i])->req = NULL;
    }
    // Starts the delay request handler thread
    if (pthread_create(&delay_req_handler, NULL, handle_delay_reqs, 
		       "Delay request handler") != 0)
    {
      printf("[ERROR] Failed to create thread: delay request handler\n");
      exit(1);
    }
    // Initializes the mutex
    if (pthread_mutex_init(&req_thread_lock, NULL) != 0)
    {
      printf("[ERROR] Failed to create mutex: request thread lock\n");
      exit(1);
    }
...
int MPID_Finalize(void)
{
    ...
    terminate_req_thread = 1;
    /** Destroys the delay request handler **/
    if (pthread_join(delay_req_handler, NULL) != 0)
    {
      printf("[ERROR] Failed to join thread: delay request handler\n");
      exit(1);
    }
    // Destroys the mutex
    pthread_mutex_destroy(&req_thread_lock);
