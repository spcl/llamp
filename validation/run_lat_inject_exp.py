...
TIMER_LIB = "/scratch/sshen/lgs-mpi-data/liballprof/mpi_timer.so"
...
def run_experiment(command: str, app: str, num_procs: int, res_file: str,
    ...
    if app == "icon":
        ...
        base_command = f"./icon-slimfly.sh {num_procs} "
    else:
        mpirun_command = f"{mpirun_bin} -np {num_procs} -f allhosts " \
                         "-envall -env OMP_NUM_THREADS 16 -env MPICH_ASYNC_PROGRESS 1 " \
    ...
    for L in range(L_min, L_max + step, step):
        ...
        total_runtime = 0
        if app == "icon":
            mpirun_command = base_command + f"{L}"
            print(f"[INFO] mpirun command to execute: {mpirun_command}")
...
if __name__ == "__main__":
    ...
    run_experiment(args.command, args.app, args.num_procs, args.res_file,
                   (L_min, L_max), args.step, args.num_trials, args.timeout)
